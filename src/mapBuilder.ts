...
export const buildMapFromSeed = (seed: number) => {
	setSeed(seed);
	...
	const getCornerCords = (roomName: string, mapArray: string[]): XYRange => { // only works for square rooms
		let coordsY: number[] = [];
		...
		mapArray.forEach((line, i) => {
			if (line.includes(roomName)) {
				...
				line.split('').forEach((c, x) => {
					if (c === roomName) { coordsXset.add(x); }
				});
			}
		});
			...
			for (let x = corners.x[0]; x <= corners.x[1]; x += 2) {
				...
				if (doubledMap[y][x] === 'z') {
					if (roomName !== 'b') {
						...
						if (x < mapFloorPlanInfo[roomName].x) { // walk left
